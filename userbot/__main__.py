...
LOGS.info(f"KEN-UBOT ⚙️ V{BOT_VER} Activated Succesfully")
    ...
    try:
        if BOTLOG_CHATID != 0:
            await bot.send_message(
                BOTLOG_CHATID,
                f"**KEN-UBOT Berhasil Di Aktifkan**\n> **Userbot Version -** `{BOT_VER}@{UPSTREAM_REPO_BRANCH}`\n> **Ketik** `{cmd}alive` **untuk Mengecheck Bot**\n",
            )
    except Exception as e:
        LOGS.info(str(e))
    try:
        await bot(JoinChannelRequest("@kennedyproject"))
        await bot(JoinChannelRequest("@emikosupport"))
