...
from userbot import APP2, APP3, APP4, APP5, bot
...
async def get_user_from_event(
    event, kenevent=None, secondgroup=None, nogroup=False, noedits=False
):
    if kenevent is None:
        kenevent = event
    if nogroup is False:
    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if previous_message.sender_id is None:
                if not noedits:
                    await edit_delete(
                        kenevent, "**ERROR: Dia adalah anonymous admin!**", 60
                    )
                return None, None
...
async def checking():
    ...
    try:
        if KEN2:
            await KEN2(Get(gocheck))
            await KEN2(Get(checker))
    except BaseException:
        pass
    try:
        if KEN3:
            await KEN3(Get(gocheck))
            await KEN3(Get(checker))
    except BaseException:
        pass
    try:
        if KEN4:
            await KEN4(Get(gocheck))
            await KEN4(Get(checker))
    except BaseException:
        pass
    try:
        if KEN5:
            await KEN5(Get(gocheck))
            await KEN5(Get(checker))
    except BaseException:
        pass
